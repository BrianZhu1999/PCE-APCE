...
def main() -> None:
    ...
    display_interior_flat = farthest_display_subset(grid_positions, interior_flat, 25)
    ...
    fig = plt.figure(figsize=(7.2, 5.95), facecolor="white")
    grid = fig.add_gridspec(
        4, 3,
        left=0.115, right=0.985, top=0.895, bottom=0.155,
    ...
    column_centers = [0.245, 0.535, 0.825]
    for x_center, label, letter in zip(column_centers, ["interior observations", "APCE", "reference"], "abc"):
        fig.text(
            x_center - 0.105, 0.922, letter,
            ha="left", va="bottom", fontsize=9.5, fontweight="bold", color="#27313A",
        )
        fig.text(
            x_center, 0.922, label,
            ha="center", va="bottom", fontsize=6.5, color="#27313A",
        )
    row_centers = [0.80, 0.615, 0.43, 0.245]
    for center, time_ms in zip(row_centers, times_ms):
        fig.text(
            0.082, center, f"{time_ms:.0f} ms", rotation=0,
            ha="right", va="center", fontsize=6.5, color="#27313A",
        )

    field_sm = ScalarMappable(norm=field_norm, cmap=field_cmap)
    field_sm.set_array([])
    cax_field = fig.add_axes([0.36, 0.092, 0.28, 0.012])
    ...
    fig.legend(
        handles=plane_handles, loc="lower center", bbox_to_anchor=(0.68, 0.055),
    ...
    fig.text(
        0.115, 0.025,
        "Central x–y, x–z and y–z planes; a shows 25 geometry-dispersed points selected from the 64 interior assimilation points.",
        ha="left", va="bottom", fontsize=5.7, color="#59636B",
    )

    args.output.mkdir(parents=True, exist_ok=True)
    stem = args.output / "s1_apce_3d_reconstruction_v5_transposed"
    ...
    source_data = args.output / "s1_apce_3d_reconstruction_v5_transposed_source_data.csv"
    ...
    registry = {
        "figure": "s1_apce_3d_reconstruction_v5_transposed",
        ...
        "panels": {
            "a": {"column": "interior observations", "source": "display subset of interior_flat", "role": "25 geometry-dispersed interior observation points"},
            "b": {"column": "APCE", "source": "mean", "role": "three-dimensional reconstruction"},
            "c": {"column": "reference", "source": "truth", "role": "full-field reference"},
        },
        "color_scale": {
        ...
        "statistics": {
            "n_definition": "25 displayed interior points in column a; formal experiment uses 64 interior and 128 boundary measurements; full 3969-grid field for columns b and c",
    ...
    (args.output / "s1_apce_3d_reconstruction_v5_transposed.provenance.json").write_text(
    ...
    (args.output / "s1_apce_3d_reconstruction_v5_transposed.qa.json").write_text(
