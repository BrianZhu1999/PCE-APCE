#!/usr/bin/env python3
"""A6 confidence-weighted and shrinkage-compensated Baoding dual tracker.
...
The A6 variant estimates node-target 2-D tangent-plane DOA covariance from
non-boundary calibration samples, shrinks the corresponding azimuth/zenith
biases, and modulates angular covariance with each frame's relative MUSIC
peak contrast. All parameters are selected inside the declared calibration
interval and frozen before held-out tracking. The input candidates are not
joint 2-D MUSIC peaks and their strengths are only relative contrast proxies;
the output is not a private Zhang-DBN reproduction or measured acoustic SNR.
"""
...
HUBER_DEG = 20.0
Q_ACCEL = 16.0
BIAS_SHRINKAGE_SAMPLES = 20.0
BIAS_ALPHA_CANDIDATES = (0.0, 0.25, 0.50, 0.75, 1.0)
QUALITY_BETA_CANDIDATES = (0.0, 0.10, 0.20, 0.35)
PRIOR_SIGMA_CANDIDATES_M = (80.0, 160.0, 320.0)
...
def circular_difference_deg(observed: float, expected: float) -> float:
...
def robust_covariance_2d(values: Sequence[np.ndarray], floor_deg: float = 2.0) -> np.ndarray:
    """Return a PSD robust covariance for tangent-plane angular residuals."""
    if len(values) < 3:
        return np.eye(2, dtype=float) * floor_deg**2
    array = np.asarray(values, dtype=float)
    centre = np.median(array, axis=0)
    residual = array - centre
    radii = np.linalg.norm(residual, axis=1)
    clip = max(float(np.percentile(radii, 80.0)), floor_deg)
    weights = np.minimum(1.0, clip / np.maximum(radii, 1e-9))
    clipped = residual * weights[:, None]
    covariance = clipped.T @ clipped / max(len(clipped) - 1, 1)
    covariance = 0.5 * (covariance + covariance.T)
    eigenvalues, eigenvectors = np.linalg.eigh(covariance)
    eigenvalues = np.clip(eigenvalues, floor_deg**2, 45.0**2)
...
def fit_quality_model(
    ...
    azimuth_residuals: Dict[Tuple[int, int], List[float]] = {(node, target): [] for node in NODES for target in TARGETS}
    zenith_residuals: Dict[Tuple[int, int], List[float]] = {(node, target): [] for node in NODES for target in TARGETS}
    tangent_residuals: Dict[Tuple[int, int], List[np.ndarray]] = {(node, target): [] for node in NODES for target in TARGETS}
    ...
    for node in NODES:
        ...
        for index, row in enumerate(bundle[node]):
            ...
            for target in TARGETS:
                ...
                # Boundary-pinned peaks are retained later as low-confidence
                # observations, but they cannot identify a systematic bias.
                if not candidate.boundary:
                    azimuth_residuals[key].append(circular_difference_deg(candidate.azimuth_deg, truths[target - 1][0]))  # type: ignore[index]
                    zenith_residuals[key].append(candidate.zenith_deg - truths[target - 1][1])  # type: ignore[index]
                    tangent_residuals[key].append(np.degrees(tangent_residual_deg(
                        (candidate.azimuth_deg, candidate.zenith_deg), truths[target - 1]  # type: ignore[arg-type]
                    )))
    ...
    azimuth_bias: Dict[Tuple[int, int], float] = {}
    zenith_bias: Dict[Tuple[int, int], float] = {}
    bias_shrinkage: Dict[Tuple[int, int], float] = {}
    tangent_covariance: Dict[Tuple[int, int], np.ndarray] = {}
    ...
    for node in NODES:
        for target in TARGETS:
            ...
            az_bias = float(np.median(azimuth_residuals[key])) if azimuth_residuals[key] else 0.0
            zen_bias = float(np.median(zenith_residuals[key])) if zenith_residuals[key] else 0.0
            bias_count = min(len(azimuth_residuals[key]), len(zenith_residuals[key]))
            shrinkage = bias_count / (bias_count + BIAS_SHRINKAGE_SAMPLES)
            azimuth_bias[key] = az_bias
            zenith_bias[key] = zen_bias
            bias_shrinkage[key] = float(shrinkage)
            tangent_covariance[key] = robust_covariance_2d(tangent_residuals[key])
            reliability_rows.append({
                ...
                "frozen_azimuth_bias_deg": az_bias,
                "frozen_zenith_bias_deg": zen_bias,
                "bias_calibration_samples": bias_count,
                "bias_shrinkage": shrinkage,
                "tangent_cov_00_deg2": float(tangent_covariance[key][0, 0]),
                "tangent_cov_01_deg2": float(tangent_covariance[key][0, 1]),
                "tangent_cov_11_deg2": float(tangent_covariance[key][1, 1]),
    ...
    model = {
        ...
        "zenith_bias_deg": zenith_bias,
        "bias_shrinkage": bias_shrinkage,
        "tangent_covariance_deg2": tangent_covariance,
        "bias_alpha": 0.0,
        "quality_beta": 0.20,
        "prior_sigma_m": 160.0,
...
def compensated_angles(
    ...
    key = (node, target)
    strength = float(model.get("bias_alpha", 0.0)) * float(model.get("bias_shrinkage", {}).get(key, 0.0))
    azimuth = (candidate.azimuth_deg - strength * float(model["azimuth_bias_deg"].get(key, 0.0))) % 360.0
    zenith = candidate.zenith_deg - strength * float(model["zenith_bias_deg"].get(key, 0.0))
    return azimuth, float(np.clip(zenith, 0.5, 89.5))
...
def sigma_for(
    ...
    if use_quality:
        ...
        beta = float(model.get("quality_beta", 0.20))
        base_sigma *= math.exp(-beta * float(np.clip((quality - centred) / scale, -3.0, 3.0)))
...
def covariance_for(
    ...
    covariance = np.asarray(
        model.get("tangent_covariance_deg2", {}).get(key, np.eye(2) * 10.0**2),
        dtype=float,
    ).copy()
    if use_quality:
        quality = quality_value(candidate, alternative)
        ...
        centred = float(model["quality_median"].get(key, 0.0))
        beta = float(model.get("quality_beta", 0.20))
        sigma_scale = math.exp(-beta * float(np.clip((quality - centred) / scale, -3.0, 3.0)))
        covariance *= sigma_scale**2
    if candidate.boundary:
        covariance *= BOUNDARY_INFLATION**2
    covariance = 0.5 * (covariance + covariance.T)
    eigenvalues, eigenvectors = np.linalg.eigh(covariance)
    eigenvalues = np.clip(eigenvalues, MIN_SIGMA_DEG**2, MAX_SIGMA_DEG**2)
    return eigenvectors @ np.diag(eigenvalues) @ eigenvectors.T


def tangent_basis(angles: Tuple[float, float]) -> np.ndarray:
    azimuth = math.radians(angles[0])
    zenith = math.radians(float(np.clip(angles[1], 0.5, 89.5)))
    e_azimuth = np.asarray([-math.sin(azimuth), math.cos(azimuth), 0.0], dtype=float)
    e_zenith = np.asarray([
        math.cos(zenith) * math.cos(azimuth),
        math.cos(zenith) * math.sin(azimuth),
        -math.sin(zenith),
    ], dtype=float)
    return np.column_stack((e_azimuth, e_zenith))
...
def weighted_rays(
    ...
    for iteration in range(6 if huber else 1):
        ...
        for item in working:
            ...
            angles = compensated_angles(item["candidate"], node, target_id, model, use_bias)
            ...
            if position is not None:
                ...
                residual = tangent_norm_deg(angles, predicted)
                residuals.append(residual)
                if huber:
                    robust_weight = min(1.0, HUBER_DEG / max(residual, 1.0e-9))
            if use_covariance:
                angular_covariance = covariance_for(
                    item["candidate"], item["alternative"], node, target_id, model, use_quality
                ) * (math.pi / 180.0) ** 2
                basis = tangent_basis(angles)
                precision = basis @ np.linalg.pinv(angular_covariance, rcond=1e-10) @ basis.T
                precision *= robust_weight / max(range_m**2, 1.0)
            else:
                sigma_deg = sigma_for(item["candidate"], item["alternative"], node, target_id, model, use_quality)
                sigma_rad = max(math.radians(sigma_deg), math.radians(MIN_SIGMA_DEG))
                weight = robust_weight / max((range_m * sigma_rad) ** 2, 1.0e-12)
                precision = weight * project_matrix(angles)
            matrix += precision
            rhs += precision @ anchor
    ...
    for item in working:
        ...
        if use_covariance:
            angular_covariance = covariance_for(
                item["candidate"], item["alternative"], node, int(item["target"]), model, use_quality
            )
            sigma = float(np.sqrt(max(np.trace(angular_covariance) / 2.0, MIN_SIGMA_DEG**2)))
        else:
    ...
    if inliers and len(inliers) < len(working):
        ...
        reduced = weighted_rays([working[index] for index in inliers], nodes, prior_position, prior_sigma_m, use_quality, use_bias, use_covariance, False, model)
...
def candidate_assignment(
    ...
    use_quality = variant in ("strength", "combined", "a2", "a3", "a6")
    use_bias = variant in ("a3", "a6")
        ...
        for permutation in ((0, 1), (1, 0)):
            ...
            for target, candidate_index in zip(TARGETS, permutation):
                ...
                if target_positions[target - 1] is None:
                    ...
                    residual = tangent_norm_deg(compensated_angles(candidate, node, target, model, use_bias), predicted_angles)
            ...
            if variant in ("a2", "a3", "a6") and frozen_priors is not None and frame_index < len(frozen_priors[node]):
...
def run_variant(
    ...
    use_motion = variant in ("combined", "a2", "a3", "a6")
    use_quality = variant in ("strength", "combined", "a2", "a3", "a6")
    use_bias = variant in ("a3", "a6")
    use_covariance = variant == "a6"
    joint_iterations = 4 if variant in ("a2", "a3", "a6") else 1
    for second in sorted(bins):
        ...
        for iteration in range(joint_iterations):
            ...
            for target in TARGETS:
                ...
                prior_sigma = float(model.get("prior_sigma_m", 160.0)) if use_motion and prior_position is not None else None
                result = weighted_rays(
                    all_observations[target], nodes, prior_position, prior_sigma,
                    use_quality, use_bias, use_covariance, variant in ("a2", "a3", "a6"), model,
                )
...
def serializable_quality_model(model: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    output: Dict[str, Any] = {}
    for name in ("node_target_sigma_deg", "quality_median", "quality_scale", "azimuth_bias_deg", "zenith_bias_deg", "bias_shrinkage", "tangent_covariance_deg2"):
        values = model.get(name, {})
        output[name] = {}
        for key, value in values.items():
            label = "node%d_target%d" % (key[0], key[1])
            output[name][label] = (
    ...
    for name in ("bias_alpha", "quality_beta", "prior_sigma_m"):
        if name in model:
            output[name] = float(model[name])
    return output
...
def main() -> None:
    ...
    # Select compensation strength only from the frozen calibration interval.
    calibration_trials: List[Dict[str, Any]] = []
    for bias_alpha in BIAS_ALPHA_CANDIDATES:
        for quality_beta in QUALITY_BETA_CANDIDATES:
            for prior_sigma_m in PRIOR_SIGMA_CANDIDATES_M:
                trial_model = dict(model)
                trial_model["bias_alpha"] = float(bias_alpha)
                trial_model["quality_beta"] = float(quality_beta)
                trial_model["prior_sigma_m"] = float(prior_sigma_m)
                trial_rows, _ = run_variant(
                    "a6", bundle, calibrations, nodes, trial_model, bins,
                    args.calibration_start_s, args.calibration_end_s, args.calibration_end_s, priors,
                )
                target_rmse = []
                for target in TARGETS:
                    split = add_metrics(trial_rows[target], tracks[target], centre, args.calibration_start_s, args.calibration_end_s, args.evaluation_end_s, delay_s)["calibration"]
                    target_rmse.append(float(split["rmse_position_m"]))
                calibration_trials.append({
                    "bias_alpha": float(bias_alpha),
                    "quality_beta": float(quality_beta),
                    "prior_sigma_m": float(prior_sigma_m),
                    "target1_calibration_rmse_m": target_rmse[0],
                    "target2_calibration_rmse_m": target_rmse[1],
                    "worst_calibration_rmse_m": max(target_rmse),
                    "mean_calibration_rmse_m": float(np.mean(target_rmse)),
                })
    selected = min(calibration_trials, key=lambda row: (row["worst_calibration_rmse_m"] + 0.2 * row["mean_calibration_rmse_m"], row["bias_alpha"], row["quality_beta"], row["prior_sigma_m"]))
    model["bias_alpha"] = selected["bias_alpha"]
    model["quality_beta"] = selected["quality_beta"]
    model["prior_sigma_m"] = selected["prior_sigma_m"]
    variant_metrics: Dict[str, Any] = {}
    variant_outputs: Dict[str, Any] = {}
    for variant in ("a2", "a6"):
    ...
    main_rows = variant_outputs["a6"]["rows"]
    for target in TARGETS:
        write_csv(args.output / ("target%d_state.csv" % target), main_rows[target])
    write_csv(args.output / "a6_assignments.csv", variant_outputs["a6"]["assignments"])
    ...
    write_json(args.output / "a6_window_audit.json", audit)
    state_manifest = {
        "task": "Baoding shuangyuan_4 A6 confidence-weighted shrinkage-compensated direct map",
        "claim_boundary": "bounded frontend confidence and DOA compensation ablation; not a Zhang DBN-LA-NM reproduction and not a physical SNR estimate",
        "nodes": list(NODES),
        ...
        "assignment": "per-node identity/swap posterior over both rank-paired candidates; A6 applies shrinkage-compensated node-target DOA bias and time-varying 2-D angular covariance",
        "variants": ["a2", "a6"],
        "motion_prior": {"model": "constant velocity", "q_accel_m2_s3": Q_ACCEL, "prior_sigma_m": model["prior_sigma_m"], "huber_deg": HUBER_DEG},
    ...
    write_json(args.output / "a6_manifest.json", state_manifest)
    write_json(args.output / "a6_metrics.json", {"variants": variant_metrics, "psd": state_manifest["covariance"]["psd"], "window_audit": audit, "selected_calibration_parameters": selected})
