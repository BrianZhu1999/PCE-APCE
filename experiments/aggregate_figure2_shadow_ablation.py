...
MECHANISM_METRICS = (
    "mechanism_mean_erasure_ratio",
    "mechanism_median_erasure_ratio",
    "mechanism_erasure_ratio_p90",
    "mechanism_mean_normalized_entropy",
    "mechanism_min_normalized_entropy",
...
def paired(rows: list[dict[str, Any]], resamples: int, seed: int) -> list[dict[str, Any]]:
    ...
    for case in CASES:
        for reference in ("apce_full", "pce_baseline"):
            ...
            for competitor in competitors:
                ...
                if not seeds:
                    continue
                for metric in METRICS + MECHANISM_METRICS:
...
def audit(rows: list[dict[str, Any]], root: Path, seed_base: int, seed_count: int) -> dict[str, Any]:
    expected = {(case, method, seed) for case in CASES for method in METHODS for seed in range(seed_base, seed_base + seed_count)}
...
def main() -> None:
    ...
    audit_payload = audit(rows, args.root, args.seed_base, args.seed_count)
