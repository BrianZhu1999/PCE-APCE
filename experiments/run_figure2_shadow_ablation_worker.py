...
from experiments import run_figure2_shadow_ablation as reviewer
...
def run_method(case: str, method: str, seed: int, device: torch.device) -> dict[str, Any]:
    ...
    result = reviewer.run_case_method_seed(case, method, seed, device, protocol=PROTOCOL)
    result.update(method=method, label=LABELS[method], implementation_method=method)
...
def main() -> None:
    ...
    for index, (case, method, seed) in enumerate(tasks, start=1):
        ...
        try:
            ...
            mechanism_arrays = row.pop("_mechanism_trace_arrays", {})
            for key, values in mechanism_arrays.items():
                trace_arrays[f"mechanism_{key}"] = np.asarray(values, dtype=float)
