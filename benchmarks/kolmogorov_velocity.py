...
DEFAULT_DATA = Path("data/kolmogorov_flow_test.npy")
WINDOW_STARTS = (10, 75, 140, 205, 270)
...
def load_truth(config: KOL64VelocityConfig) -> np.ndarray:
    ...
    if not data_path.exists():
        raise FileNotFoundError(f"Kolmogorov data not found: {data_path}")
    ...
    if abs(RE_LEVELS[re_index] - config.reynolds) > 1e-8 or K_LEVELS[k_index] != config.forcing_wavenumber:
        raise ValueError("The dataset does not contain the requested Reynolds number and forcing wavenumber")
    ...
    if segment.shape[-1] != 2:
        raise ValueError("Kolmogorov data must contain two velocity components")
    return segment
...
def materialize(shared: SharedAssets, device: torch.device) -> Scenario:
    ...
    return Scenario(config, shared.truth, observations, initial_ensemble, shared.forecast_noise, shared.sensor_indices, shared.sensor_spatial_indices, physical, augmented, sigma, shared.velocity_rms, shared.asset_path)
...
def run_one(args: argparse.Namespace) -> None:
    ...
    payload: dict[str, Any] = {
        "protocol": "kolmogorov-velocity-assimilation",
        ...
        "window_end": cfg.window_start + cfg.steps,
        "reynolds": cfg.reynolds,
        "forcing_wavenumber": cfg.forcing_wavenumber,
        "alpha_true": cfg.alpha_true,
        "sensor_grid": cfg.sensor_grid,
        "observed_points_uv": cfg.observed_points,
        "obs_interval": cfg.observation_interval,
        "steps": cfg.steps,
        "save_dt": cfg.save_dt,
        "state_dim": cfg.state_dim,
        "data_path": cfg.data_path,
        "trace_npz": trace,
        "runtime_total_seconds": time.perf_counter() - started_total,
        **{key: value for key, value in result.items() if isinstance(value, (int, float, str))},
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run sparse velocity assimilation for Kolmogorov flow.")
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--method", choices=METHODS, required=True)
    parser.add_argument("--seed", type=int, required=True)
    parser.add_argument("--sensor-grid", type=int, choices=(8, 16), default=16)
    parser.add_argument("--window-start", type=int, required=True)
    parser.add_argument("--reynolds", type=float, choices=RE_LEVELS, default=1500.0)
    parser.add_argument("--forcing-wavenumber", type=int, choices=K_LEVELS, default=2)
    parser.add_argument("--observation-interval", type=int, choices=(1, 2, 4, 6, 8), default=4)
