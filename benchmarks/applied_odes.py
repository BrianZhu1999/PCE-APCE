...
from pce_assimilation.systems.base import HybridSystem
...
def load_protocol_config(config_path: Path) -> dict[str, Any]:
    """Load and validate the published PCE/APCE settings."""

    if not config_path.is_file():
        raise FileNotFoundError(f"Missing protocol configuration: {config_path}")
    config = json.loads(config_path.read_text(encoding="utf-8"))
    config.setdefault("method_overrides", {})
    config.setdefault("case_overrides", {})
    allowed = set(CaseConfig.__dataclass_fields__)
    for method, overrides in config["method_overrides"].items():
        if method not in {"pce", "apce"}:
            raise ValueError(f"Unsupported method in protocol configuration: {method}")
        unknown = sorted(set(overrides) - allowed)
        if unknown:
            raise ValueError(f"Unsupported {method} fields: {unknown}")
    for case, overrides in config["case_overrides"].items():
        if case not in CASES:
            raise ValueError(f"Unknown case in protocol configuration: {case}")
        for key, values in overrides.items():
            if key in {"both", "pce", "apce"}:
                unknown = sorted(set(values) - allowed)
            else:
                unknown = [] if key in allowed else [key]
            if unknown:
                raise ValueError(f"Unsupported fields for {case}: {unknown}")
    return config
...
def apply_protocol_config(
    ...
    overrides.update(protocol.get("method_overrides", {}).get(method, {}))
    case_overrides = protocol.get("case_overrides", {}).get(case, {})
    overrides.update({key: value for key, value in case_overrides.items() if key not in {"both", "pce", "apce"}})
    overrides.update(case_overrides.get("both", {}))
    overrides.update(case_overrides.get(method, {}))
    return replace(config, **_normalize_config_overrides(overrides)) if overrides else config

def make_system(config: CaseConfig) -> HybridSystem:
    return applied_ode_case_spec(config.name).system_factory()


def initial_state(config: CaseConfig, device: torch.device) -> torch.Tensor:
    return applied_ode_case_spec(config.name).initial_state_factory(device)


def initial_scale(config: CaseConfig) -> torch.Tensor:
    return applied_ode_case_spec(config.name).initial_scale_factory()


def observation_indices(config: CaseConfig, device: torch.device) -> torch.Tensor:
    return applied_ode_case_spec(config.name).observation_indices_factory(device)
...
def pce_state_output_weights(
    ...
    estimator. This layer controls how sharply the branch ensemble contributes
    to state metrics and downstream traces.
    """
...
def estimate_physical_metrics(config: CaseConfig, estimate: np.ndarray, truth: np.ndarray) -> dict[str, float]:
    nonnegative_cases = {"chemical", "pk_infusion", "robertson"}
    ...
    if config.name in {"fhn", "pendulum"}:
...
def run_one(
    ...
    device: torch.device,
    record_trace: bool,
    protocol_config_values: dict[str, Any],
) -> dict[str, Any]:
    started = time.perf_counter()
    config = apply_protocol_config(config_for_case(case, seed), protocol_config_values, case, method)
    ...
    row = {
        ...
        "protocol": "applied-ode-assimilation",
        "seed": seed,
    ...
    *,
    ...
    if output.is_file() and not force:
        ...
        if (
            ...
            and row.get("protocol") == "applied-ode-assimilation"
            and (not record_trace or trace_output.exists())
        ):
            return row
    output.parent.mkdir(parents=True, exist_ok=True)
    try:
        row = run_one(case, method, seed, device, record_trace=record_trace, protocol_config_values=protocol_config_values)
...
def save_traces(output: Path, trace_rows: list[dict[str, Any]]) -> None:
    if not trace_rows:
        return
    ...
    np.savez_compressed(output / "applied_odes_traces.npz", **arrays)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the applied uncertain ODE benchmark.")
    ...
    parser.add_argument("--protocol-config", type=Path, default=PROJECT_ROOT / "benchmarks" / "applied_odes_protocol.json")
    parser.add_argument("--output", type=Path, default=Path("results/applied_odes"))
    ...
    args.methods = parse_csv_choices(args.methods, METHODS)
    if args.n_seeds < 1:
...
def main() -> None:
    args = parse_args()
    args.protocol_config_values = load_protocol_config(args.protocol_config)
    device = torch.device(args.device)
    if args.single_case and args.single_method and args.single_seed is not None:
        output = args.single_output or (args.output / "runs" / f"{args.single_case}_{args.single_method}_s{args.single_seed}.json")
        row = run_json_path(
            output,
            args.single_case,
            args.single_method,
            ...
            protocol_config_values=args.protocol_config_values,
        )
    ...
    for case in args.cases:
        for seed_offset in range(args.n_seeds):
            ...
            for method in args.methods:
                completed += 1
                record_trace = args.record_trace and seed_offset == 0
                path = args.output / "runs" / f"{case}_{method}_s{seed}.json"
                row = run_json_path(
                    path,
                    case,
                    method,
    ...
    write_csv(source_data / "applied_odes_run_source_data.csv", records)
    summary = summarize(records, args.base_seed)
    comparisons = paired_comparisons(records, args.base_seed + 10_000)
    write_csv(source_data / "applied_odes_method_summary.csv", summary)
    write_csv(source_data / "applied_odes_paired_comparisons.csv", comparisons)
    write_summary_json(args.output, args, records, summary)
    save_traces(source_data, trace_rows)
