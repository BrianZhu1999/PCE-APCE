...
DEFAULT_INPUT = Path("data/KSE_test.npy")
...
def sample_true_mu(sample_index: int) -> float:
    # The KSE_test.npy dataset contains three unseen ICs for each of the main-text
    # visual/test parameters 1.1, 2.5 and 3.2, in repeating order.  This mapping
    # was verified by one-step ETDRK4 prediction against the published dataset.
...
def make_observation_operator(config: KSEConfig, observation_indices: torch.Tensor) -> SparseObservation:
    return SparseObservation(observation_indices)


def observation_covariance(scenario: KSEScenario, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
...
def generate_scenario(config: KSEConfig, input_path: Path, device: torch.device) -> KSEScenario:
    ...
    observation_scale = max(truth_std, 1.0e-12)
...
def trace_common_payload(scenario: KSEScenario) -> dict[str, Any]:
...
def evidence_gap_state_weights(base_weights: torch.Tensor, log_scores: torch.Tensor, config: KSEConfig) -> torch.Tensor:
...
def pce_point_estimate(mu_grid: torch.Tensor, branch_means: torch.Tensor, state_weights: torch.Tensor, log_scores: torch.Tensor, config: KSEConfig) -> torch.Tensor:
...
def run_aug_enkf(scenario: KSEScenario, device: torch.device, *, record_trace: bool = False) -> dict[str, Any]:
    config = scenario.config
...
def run_bma(scenario: KSEScenario, device: torch.device, *, record_trace: bool = False) -> dict[str, Any]:
    config = scenario.config
    solver = KSEETDRK4(state_dim=config.state_dim, length=config.length, sub_dt=config.saved_dt / config.solver_substeps, device=device, dtype=torch.float64)
...
def run_pce_apce(scenario: KSEScenario, method: MethodName, device: torch.device, *, record_trace: bool = False) -> dict[str, Any]:
    config = scenario.config
    solver = KSEETDRK4(state_dim=config.state_dim, length=config.length, sub_dt=config.saved_dt / config.solver_substeps, device=device, dtype=torch.float64)
    ...
    for step in range(config.steps + 1):
        ...
        if refined.shape != mu_grid.shape or not torch.allclose(refined, mu_grid):
            ...
            state_weights = evidence_gap_state_weights(path_weights, log_scores, config)
            regrid_count += 1
        # Local branch analysis is paired with member-wise and global weighted
        # state-parameter correction; shadow paths remain analysis-isolated.
...
def run_one(method: MethodName, scenario: KSEScenario, device: torch.device, record_trace: bool) -> dict[str, Any]:
    if method == "aug_enkf":
...
def write_summary(output_root: Path) -> None:
    ...
    groups = sorted(
        {
            (
                int(row.get("downsampling_factor", 32)),
                int(row.get("temporal_obs_interval", 2)),
            )
            for row in rows
        }
    )
    present_methods = [method for method in METHODS if any(row.get("method") == method for row in rows)]
    for factor, temporal in groups:
        for method in present_methods:
            subset = [
                row
                for row in rows
                if int(row.get("downsampling_factor", 32)) == factor
                and int(row.get("temporal_obs_interval", 2)) == temporal
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the published Kuramoto-Sivashinsky benchmark.")
    ...
    temporal_tag = f"time{int(args.temporal_obs_interval)}"
    run_id = f"ks_space{args.downsampling_factor}_{temporal_tag}_{args.method}_seed{args.seed_index:02d}_sample{sample_index:02d}"
