...
def panel_heat(ax: plt.Axes, data: np.lib.npyio.NpzFile) -> None:
    ...
    inset = ax.inset_axes([0.55, 0.14, 0.40, 0.36])
    ...
    mark_inset(ax, inset, loc1=2, loc2=4, fc="none", ec="#B8B8B8", linewidth=0.60)
    polish_axis(ax)
...
f-k: six seed-wise boxplots for nRMSE and CRPS across Wave, Spring and Heat.
l-q: six mean-and-95% CI bar charts for the same metrics and systems.

Statistics:
n=50 paired seeds per system and method. Boxplots show seed-level distributions. Bars show mean with 95% CI. APCE is highlighted by a red frame in statistical panels.
"""
    (output_dir / "figure2_classical_uncertain_systems_v8_566_contract.md").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    letters_box = iter("fghijk")
    letters_bar = iter("lmnopq")
    slot = 0
    for case in CASES:
    ...
    base = args.output_dir / "figure2_classical_uncertain_systems_v8_566"
    ...
    qa = {
        "figure": "figure2_classical_uncertain_systems_v8_566",
        "layout": "strict three rows: 5 + 6 + 6 panels",
        "backend": "python/matplotlib",
        "font": "Arial with sans-serif fallback",
        "font_weight": "regular",
        "formats": ["svg", "pdf", "png", "tiff"],
        "statistics": "n=50 paired seeds; APCE red frame in box and bar panels",
        "panels": list("abcdefghijklmnopq"),
    }
    (args.output_dir / "figure2_classical_uncertain_systems_v8_566_qa.json").write_text(json.dumps(qa, indent=2, ensure_ascii=False), encoding="utf-8")
