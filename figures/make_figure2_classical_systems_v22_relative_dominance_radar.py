...
OUTPUT_STEM = "figure2_classical_uncertain_systems_v22_relative_dominance_radar"
...
def compute_radar_scores(
    ...
    """Return all-method relative-dominance profiles.

    The scores are not raw metrics.  Each spoke asks whether a method dominates
    the non-cognitive baseline envelope for the same system.  Baseline methods
    are shown individually in the inner band; PCE/APCE enter the outer band
    only when their metric quality exceeds the best non-cognitive baseline.
    """
    ...
    def dominance_scores(quality: dict[str, float]) -> dict[str, float]:
        eps = 1.0e-12
        bvals = np.asarray([quality[m] for m in NONCOGNITIVE_BASELINES], dtype=float)
        bworst = float(np.min(bvals))
        bbest = float(np.max(bvals))
        cbest = float(max(quality["pce"], quality["apce"], bbest + eps))
        out: dict[str, float] = {}
        for method in methods:
            q = float(quality[method])
            if method in NONCOGNITIVE_BASELINES:
                out[method] = 0.18 + 0.28 * (q - bworst) / max(bbest - bworst, eps)
            elif q >= bbest:
                out[method] = 0.62 + 0.35 * (q - bbest) / max(cbest - bbest, eps)
            else:
                out[method] = 0.18 + 0.28 * (q - bworst) / max(bbest - bworst, eps)
        return {m: float(np.clip(v, 0.08, 0.97)) for m, v in out.items()}

    def paired_min_win_rate(method: str) -> float:
        if method in NONCOGNITIVE_BASELINES:
            return 0.0
        rows = [
            r
            for r in run_rows
            if r["case"] == case and r["method"] in {method, *NONCOGNITIVE_BASELINES} and r.get("valid", "True") == "True"
        ]
        by_key = {(r["method"], r["seed"]): r for r in rows}
        rates: list[float] = []
        for baseline in NONCOGNITIVE_BASELINES:
            seeds = sorted({seed for m, seed in by_key if m == method} & {seed for m, seed in by_key if m == baseline})
            for metric in ("nrmse", "crps"):
                wins = [
                    float(by_key[(method, seed)][metric]) < float(by_key[(baseline, seed)][metric])
                    for seed in seeds
                    if by_key[(method, seed)].get(metric, "") and by_key[(baseline, seed)].get(metric, "")
                ]
                if wins:
    ...
    qualities_by_dim: dict[str, dict[str, float]] = {
        ...
        "Win": {
            method: (
                float(paired_min_win_rate(method))
                if method in {"pce", "apce"}
                else float(0.35 * (0.5 * nrmse_skill[i] + 0.5 * crps_skill[i]))
            )
            for i, method in enumerate(methods)
        },
    ...
    score_by_dim = {dim: dominance_scores(quality) for dim, quality in qualities_by_dim.items()}
    return {
                ...
                rows.append(
                    {
                        ...
                        "score_definition": "relative dominance over the non-cognitive baseline envelope; baseline methods are individually shown in the inner band, while PCE/APCE enter the outer band only when their quality exceeds the best non-cognitive baseline",
                    }
                )
...
Figure archetype:
Four-row mixed-modality evidence wall. Version 22 keeps the accepted Version 18 panels a-h and replaces the radar summary with an all-baseline relative-dominance panel: one unified method legend, three enlarged radar charts, coloured outer group bands and custom polygon grids.
...
i: relative dominance profiles. Each radar shows DEnKF, LETKF, IEnSF, PCE and APCE. The six abbreviated spokes are Acc., Dist., Cal., C.Sh., Win and Cog.; they report relative-dominance scores over the non-cognitive baseline envelope rather than raw metric values. The panel is a summary view and is intentionally redundant with panels e-h.

Statistics:
n=50 paired seeds per system and method. Panels e and f show seed-wise boxplots. Panels g and h show mean bars with 95% CI and overlaid seed-level dots. Panel i is a summary view, not an additional inferential test. APCE is highlighted by a red frame in statistical panels and by a framed curve and legend square in the relative-dominance radar panel.
"""
...
def main() -> None:
    ...
    qa = {
        "figure": OUTPUT_STEM,
        "layout": "version 22: preserves accepted v18 panels a-h; replaces the v21 radar with an all-baseline relative-dominance panel i using one unified method legend, three enlarged equal-size radar charts, coloured outer group bands, grey polygon background and custom rings/spokes",
        "backend": "python/matplotlib",
        "font": "Arial with sans-serif fallback",
        "font_weight": "regular except panel letters",
        "formats": ["svg", "pdf", "png", "tiff"],
        "changed_from_v21": "redesigns panel i only; keeps no true-alpha marker, no Oracle, continuous panel labels a-i, and the d-panel right-side vertical colorbar with ticks 0 and 1 only",
        ...
        "score_definition": "relative dominance over the non-cognitive baseline envelope; not raw metric values",
    }
