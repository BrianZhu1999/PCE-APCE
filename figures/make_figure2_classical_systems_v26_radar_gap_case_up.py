...
OUTPUT_STEM = "figure2_classical_uncertain_systems_v26_radar_gap_case_up"
...
def draw_single_radar(
    ...
    for group_label, start, end, color in RADAR_GROUPS:
        ...
        if group_label == "Reliability tests":
            theta_start -= step * 0.045
            theta_end += step * 0.045
...
def add_radar_abbreviation_key(ax: plt.Axes) -> None:
    ax.set_axis_off()
    ...
    ax.add_patch(box)
    key_text = (
        "Acc. accuracy     Dist. distribution     Cal. calibration     "
        "C.Sh. concentration     Win paired wins     Cog. cognitive inference"
    )
    ax.text(
        0.50,
        0.51,
...
def panel_radar_summary(
    ...
    panel_grid = GridSpecFromSubplotSpec(3, 1, subplot_spec=subplot_spec, height_ratios=[0.105, 1.00, 0.145], hspace=-0.035)
    ...
    add_radar_abbreviation_key(fig.add_subplot(panel_grid[2, 0]))
...
def main() -> None:
    ...
    outer = fig.add_gridspec(
        8,
        1,
        height_ratios=[1.03, 0.13, 0.11, 0.92, 0.28, 0.92, 0.09, 1.84],
        hspace=0.19,
        left=0.050,
        right=0.994,
        top=0.955,
    ...
    qa = {
        "figure": OUTPUT_STEM,
        "layout": "version 26: preserves accepted panels a-h; reduces the gap above panel i; moves radar case labels upward; widens the outer radar bands, especially Reliability tests; adds a compact abbreviation key below the radar panel",
