...
OUTPUT_BASE = HERE / "figure4_kol_quant_panel_n_v2_bottomsafe"
...
def isotropic_energy_spectrum(states: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    energy_density = 0.5 * (np.abs(ux_hat) ** 2 + np.abs(uy_hat) ** 2) / (n * n) ** 2
...
def main() -> None:
    ...
    qa = {
        ...
        "metrics": ["nrmse", "crps"],
        "energy_spectrum": "2D isotropic kinetic-energy spectrum from ux, uy mean-state traces; truth from the same representative seed",
