...
"""
    (output_dir / "figure2_classical_uncertain_systems_v10_566_compact_contract.md").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    row2 = GridSpecFromSubplotSpec(1, 6, subplot_spec=outer[1, 0], wspace=0.22)
    row3 = GridSpecFromSubplotSpec(1, 6, subplot_spec=outer[2, 0], wspace=0.22)
    slot = 0
    for case in CASES:
    ...
    base = args.output_dir / "figure2_classical_uncertain_systems_v10_566_compact"
    ...
    qa = {
        "figure": "figure2_classical_uncertain_systems_v10_566_compact",
        "layout": "strict three visual rows: first row a-e; second row one grouped panel f with six small multiples; third row one grouped panel g with six small multiples; width scaled to 0.9 of v9 and column spacing tightened",
    ...
    (args.output_dir / "figure2_classical_uncertain_systems_v10_566_compact_qa.json").write_text(json.dumps(qa, indent=2, ensure_ascii=False), encoding="utf-8")
