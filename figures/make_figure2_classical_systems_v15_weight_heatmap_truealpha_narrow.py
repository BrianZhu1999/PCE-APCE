...
OUTPUT_STEM = "figure2_classical_uncertain_systems_v15_weight_heatmap_truealpha_narrow"
TRUE_ALPHA = 0.12
...
def panel_weight_heatmap(
    ...
    base.add_panel(ax, "e", "Cognitive-weight map")
    ...
    true_x = _alpha_to_heatmap_x(TRUE_ALPHA, common_alpha)
    ax.axvline(true_x, color="#272727", lw=1.0, ls=(0, (2.0, 1.4)), zorder=8, clip_on=False)
    ax.scatter([true_x], [-0.18], marker="v", s=22, color="#272727", zorder=9, clip_on=False)
    ax.text(
        true_x + 0.08,
        -0.36,
        r"true $\alpha$",
    ...
    cbar.set_label("weight", fontsize=base.AXIS_LABEL_FONT_SIZE, labelpad=3)
...
Figure archetype:
Strict three-row mixed-modality evidence wall. Version 15 returns to the Version 13 cognitive-weight heatmap structure, with a clearer low-weight color scale, a true-alpha marker, a slightly narrower canvas and a slightly narrower e column.

Panel map:
a-c: representative dynamics.
e: final cognitive-weight map over candidate alpha values for PCE and APCE across Wave, Spring and Heat, with true alpha marked.
...
def main() -> None:
    ...
    for row_index, row_metrics in enumerate((base.METRIC_GROUPS[:2], base.METRIC_GROUPS[2:])):
    ...
    qa = {
        "figure": OUTPUT_STEM,
        "layout": "version 15: first row a-c plus a slightly narrower e panel; e keeps the version 13 cognitive-weight heatmap structure with a beige low-weight color scale, colorbar border, and true-alpha marker; second row f/g; third row h/i",
        "backend": "python/matplotlib",
        "font": "Arial with sans-serif fallback",
        "font_weight": "regular except panel letters",
        "formats": ["svg", "pdf", "png", "tiff"],
        "changed_from_v13": "keeps the v13 heatmap structure; changes low-weight color from near-white to beige, adds colorbar border and true-alpha marker, narrows figure width from 12.15 to 11.55 inches, and slightly narrows e column",
