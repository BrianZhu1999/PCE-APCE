...
OUTPUT_BASE = HERE / "figure4_kol_quant_panel_n_v9_ymins_reflegend"
...
def draw_metric_boxplot(ax: plt.Axes, rows: list[dict[str, str]], metric: str, title: str, ymin: float, ymax: float) -> None:
...
def draw_energy_spectrum(ax: plt.Axes) -> dict[str, str]:
    ...
    inset = ax.inset_axes([0.62, 0.53, 0.34, 0.43])
    style_map = {
    ...
    # Keep the inset x axis linear over the narrow lambda=10-15 window so
    # tick labels stay legible in the final multi-panel composite.
    inset.set_yscale("log")
    inset.set_xlim(10, 15)
    high_vals = np.concatenate([spec[(lam >= 10) & (lam <= 15)] for lam, spec in spectra.values()])
...
def main() -> None:
    ...
    draw_metric_boxplot(ax_nrmse, rows, "nrmse", "nRMSE", ymin=0.02, ymax=0.08)
    draw_metric_boxplot(ax_crps, rows, "crps", "CRPS", ymin=0.01, ymax=0.06)
    ...
    qa = {
        ...
        "metrics": ["nrmse", "crps"],
        "energy_spectrum": "2D isotropic kinetic-energy spectrum from ux, uy mean-state traces; reference from the same representative seed; inset enlarged over lambda=10-15",
