...
OUTPUT_STEM = "figure2_classical_uncertain_systems_v20_radar_reference_style"
...
RADAR_METRICS = (
    ("nrmse_mean", "nRMSE", "low"),
    ("rmse_mean", "RMSE", "low"),
    ("crps_mean", "CRPS", "low"),
    ("coverage_90_mean", "Coverage", "target_0.90"),
    ("interval_width_90_mean", "Sharpness", "low"),
    ("nrmse_sd", "Stability", "low"),
)

RADAR_GROUPS = (
    ("Error tests", 0, 1, "#D98DA7"),
    ("Probabilistic tests", 2, 3, "#E8B96B"),
...
def draw_single_radar(
    ...
    ax.fill(angles_closed, np.full_like(angles_closed, 1.0), color="#DDE1E4", alpha=0.58, zorder=-3)
    ...
    for method in base.METHODS:
        ...
        if method == "apce":
            ax.fill(angles_closed, values, color=base.COLORS[method], alpha=0.15, zorder=3)
            ...
            ax.fill(angles_closed, values, color=base.COLORS[method], alpha=0.055, zorder=2)
            ax.plot(angles_closed, values, color=base.COLORS[method], lw=0.95, alpha=0.88, zorder=6)
        else:
            ax.plot(angles_closed, values, color=base.COLORS[method], lw=0.82, alpha=0.74, zorder=5)
...
def panel_radar_summary(
    ...
    panel_grid = GridSpecFromSubplotSpec(2, 1, subplot_spec=subplot_spec, height_ratios=[0.18, 1.00], hspace=0.02)
    ...
    radar_grid = GridSpecFromSubplotSpec(1, 3, subplot_spec=panel_grid[1, 0], wspace=0.08)
...
Figure archetype:
Four-row mixed-modality evidence wall. Version 20 keeps the accepted Version 18 panels a-h and replaces the radar summary with a reference-style design: a unified method legend, coloured outer group bands and custom polygon grids.
...
i: summary performance profiles. Each radar converts nRMSE, RMSE, CRPS, coverage calibration, interval sharpness and seed stability to higher-is-better scores in [0, 1]. The six spokes are grouped into Error tests, Probabilistic tests and Reliability tests.

Statistics:
n=50 paired seeds per system and method. Panels e and f show seed-wise boxplots. Panels g and h show mean bars with 95% CI and overlaid seed-level dots. Panel i is a summary view, not an additional inferential test. APCE is highlighted by a red frame in statistical panels and by a framed curve and legend square in the radar panel.
"""
...
def main() -> None:
    ...
    fig = plt.figure(figsize=(11.55, 12.55))
    outer = fig.add_gridspec(
        8,
        1,
        height_ratios=[1.03, 0.13, 0.11, 0.92, 0.28, 0.92, 0.40, 1.36],
        hspace=0.22,
        left=0.050,
        right=0.994,
        top=0.955,
    ...
    qa = {
        "figure": OUTPUT_STEM,
        "layout": "version 20: preserves accepted v18 panels a-h; replaces the v19 radar with a reference-style fourth-row panel i using a unified method legend, three equal-size radar charts, coloured outer group bands, grey polygon background and custom rings/spokes",
        "backend": "python/matplotlib",
        "font": "Arial with sans-serif fallback",
        "font_weight": "regular except panel letters",
        "formats": ["svg", "pdf", "png", "tiff"],
        "changed_from_v19": "restyles panel i only; keeps no true-alpha marker, no Oracle, continuous panel labels a-i, and the d-panel right-side vertical colorbar with ticks 0 and 1 only",
