...
OUTPUT_STEM = "figure2_classical_uncertain_systems_v29_radar_outer_band_no_overlap"
...
def panel_radar_summary(
    ...
    panel_grid = GridSpecFromSubplotSpec(2, 1, subplot_spec=subplot_spec, height_ratios=[0.145, 1.00], hspace=-0.020)
    ...
    for index, case in enumerate(base.CASES):
        ...
        dx = 0.016 if index == 2 else (-0.016 if index == 0 else 0.0)
        x0 = bbox.x0 + 0.5 * (bbox.width - width) + dx
        y0 = bbox.y0 + 0.5 * (bbox.height - height) - 0.012
...
def main() -> None:
    ...
    outer = fig.add_gridspec(
        8,
        1,
        height_ratios=[1.03, 0.13, 0.11, 0.92, 0.28, 0.92, 0.06, 1.90],
        hspace=0.19,
        left=0.050,
        right=0.994,
        top=0.955,
    ...
    qa = {
        "figure": OUTPUT_STEM,
        "layout": "version 29: preserves accepted panels a-h; removes the abbreviation key; expands panel-i outer radar bands outward while keeping the inner edge outside the unit circle; enlarges the radar charts without overlap between panel-i title, legend and radial labels",
