...
# Balanced mid-tone tints: a touch stronger than v61, still unobtrusive.
L96_BACKGROUND_SOFT = "#F9F8FF"
KSE_BACKGROUND_SOFT = "#F8FCFA"
KOL_BACKGROUND_SOFT = "#FFF9F3"


def apply_v62_axes_style(
    fig: plt.Figure,
    *,
    background: str,
    linewidth_factor: float = LINEWIDTH_FACTOR,
) -> None:
    """Apply the Figure 4 v62 visual-only style before export.
    ...
    for ax in fig.axes:
        ...
        # 3D axes store grid panes separately; make them a bit less washed out
        # than the v61 set while still keeping the panel feeling light.
        for axis_name in ("xaxis", "yaxis", "zaxis"):
            axis = getattr(ax, axis_name, None)
            pane = getattr(axis, "pane", None)
            if pane is not None:
                try:
                    pane.set_facecolor(mpl.colors.to_rgba(background, 0.65))
