...
def add_panel(ax: plt.Axes, letter: str, title: str) -> None:
    ax.text(-0.085, 1.055, letter, transform=ax.transAxes, ha="left", va="bottom", fontsize=10.0, fontweight="bold")
...
def add_metric_group_label(ax: plt.Axes, letter: str, title: str) -> None:
    ax.text(-0.115, 1.175, letter, transform=ax.transAxes, ha="left", va="bottom", fontsize=10.0, fontweight="bold")
    ax.text(-0.020, 1.175, title, transform=ax.transAxes, ha="left", va="bottom", fontsize=8.0)
...
def method_line_handles(include_truth: bool = True) -> list[Line2D]:
    handles: list[Line2D] = []
    if include_truth:
        handles.append(Line2D([0], [0], color=COLORS["truth"], lw=1.25, label="Truth"))
    handles += [
        Line2D([0], [0], color=COLORS[m], lw=1.05 if m in {"pce", "apce"} else 0.82, alpha=BASELINE_ALPHA[m], label=METHOD_LABELS[m])
        for m in METHODS
    ]
    return handles
...
def add_local_method_legend(ax: plt.Axes, x_anchor: float = 0.56) -> None:
    ax.legend(
        handles=method_line_handles(include_truth=True),
        loc="upper center",
...
def panel_calibration(ax: plt.Axes, summary: dict[tuple[str, str], dict[str, str]]) -> None:
    ...
    case_legend = ax.legend(case_handles, [CASE_LABELS[c] for c in CASES], loc="upper left", fontsize=6.4, ncol=3, handlelength=0.75, handletextpad=0.25, columnspacing=0.55, borderpad=0.05)
    ...
    ax.legend(method_handles, [METHOD_LABELS[m] for m in METHODS], loc="lower left", bbox_to_anchor=(0.00, 0.135), fontsize=5.8, ncol=2, handlelength=0.65, handletextpad=0.25, columnspacing=0.55, borderpad=0.03, labelspacing=0.10)
...
def metric_case_panel(
    ...
    ax.set_ylabel(ylabel if show_ylabel else "", fontsize=7.2)
    ax.set_facecolor("#FAFAFA")
    polish_axis(ax, tick_size=6.6, label_size=7.2)
...
"""
    (output_dir / "figure2_classical_uncertain_systems_v11_case_metric_fghi_contract.md").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    fig = plt.figure(figsize=(12.15, 7.25))
    outer = fig.add_gridspec(
        4,
        1,
        height_ratios=[1.03, 0.13, 0.92, 0.92],
        hspace=0.34,
        left=0.050,
        right=0.994,
        top=0.945,
    ...
    top = GridSpecFromSubplotSpec(1, 5, subplot_spec=outer[0, 0], wspace=0.22, width_ratios=[1.05, 1.05, 1.05, 0.88, 1.02])
    ...
    row2 = GridSpecFromSubplotSpec(1, 6, subplot_spec=outer[2, 0], wspace=0.18)
    row3 = GridSpecFromSubplotSpec(1, 6, subplot_spec=outer[3, 0], wspace=0.18)
    for row_index, row_metrics in enumerate((METRIC_GROUPS[:2], METRIC_GROUPS[2:])):
        ...
        for metric_index, (metric, ylabel, scale, letter, title) in enumerate(row_metrics):
            for case_index, case in enumerate(CASES):
                slot = metric_index * len(CASES) + case_index
                ax = fig.add_subplot(row_spec[0, slot])
    ...
    base = args.output_dir / "figure2_classical_uncertain_systems_v11_case_metric_fghi"
    ...
    qa = {
        "figure": "figure2_classical_uncertain_systems_v11_case_metric_fghi",
        "layout": "strict three visual rows with a dedicated method legend strip between the first and second rows: first row a-e with local legends in a-c and method-color legend in d; second row f=nRMSE boxplots and g=CRPS boxplots, each ordered Wave/Spring/Heat; third row h=90% coverage bars and i=interval width bars, each ordered Wave/Spring/Heat",
    ...
    (args.output_dir / "figure2_classical_uncertain_systems_v11_case_metric_fghi_qa.json").write_text(json.dumps(qa, indent=2, ensure_ascii=False), encoding="utf-8")
