...
OUTPUT_STEM = "figure2_classical_uncertain_systems_v19_with_radar_summary"
...
RADAR_METRICS = (
    ...
    ("rmse_mean", "RMSE\nskill", "low"),
    ("crps_mean", "CRPS\nskill", "low"),
    ("coverage_90_mean", "Coverage\ncalibration", "target_0.90"),
    ("interval_width_90_mean", "Interval\nsharpness", "low"),
    ("nrmse_sd", "Seed\nstability", "low"),
)
...
def compute_radar_scores(
    ...
    method_scores = {method: [] for method in base.METHODS}
    for field, _, mode in RADAR_METRICS:
        raw = np.asarray([float(summary[(case, method)][field]) for method in base.METHODS], dtype=float)
        scores = _score_higher_better(raw, mode)
        for method, score in zip(base.METHODS, scores, strict=True):
            method_scores[method].append(float(score))
    return {method: np.asarray(scores, dtype=float) for method, scores in method_scores.items()}
...
def draw_single_radar(
    ...
    ax.set_xticklabels(labels, fontsize=base.AXIS_TICK_FONT_SIZE)
    ...
    for radius in (0.25, 0.50, 0.75, 1.00):
        ax.plot(
            ...
            color="#DCDCDC",
            lw=0.55 if radius < 1.0 else 0.75,
            zorder=0,
        )
    for angle in angles:
        ax.plot([angle, angle], [0.0, 1.0], color="#E7E7E7", lw=0.50, zorder=0)
    ax.text(np.deg2rad(86), 0.50, "0.5", fontsize=base.AXIS_TICK_FONT_SIZE, color="#6E6E6E", ha="left", va="center")
    ax.text(np.deg2rad(86), 1.00, "1", fontsize=base.AXIS_TICK_FONT_SIZE, color="#6E6E6E", ha="left", va="center")

    for method in base.METHODS:
        values = np.r_[scores[method], scores[method][0]]
        if method == "apce":
            ax.fill(angles_closed, values, color=base.COLORS[method], alpha=0.16, zorder=3)
            ax.plot(angles_closed, values, color=base.APCE_FRAME, lw=2.10, alpha=0.96, zorder=7)
            ax.plot(angles_closed, values, color=base.COLORS[method], lw=1.10, alpha=1.0, zorder=8)
        elif method == "pce":
            ax.fill(angles_closed, values, color=base.COLORS[method], alpha=0.07, zorder=2)
            ax.plot(angles_closed, values, color=base.COLORS[method], lw=1.10, alpha=0.85, zorder=5)
        else:
            ax.plot(angles_closed, values, color=base.COLORS[method], lw=0.82, alpha=0.42, zorder=4)

    ax.set_title(case_label, fontsize=base.PANEL_TITLE_FONT_SIZE, fontweight="regular", pad=9.0)
...
def panel_radar_summary(
    ...
    label_ax = fig.add_subplot(subplot_spec)
    ...
    radar_grid = GridSpecFromSubplotSpec(1, 3, subplot_spec=subplot_spec, wspace=0.32)
    for index, case in enumerate(base.CASES):
        ax = fig.add_subplot(radar_grid[0, index], projection="polar")
        draw_single_radar(ax, compute_radar_scores(summary, case), base.CASE_LABELS[case])
...
def write_radar_source_data(
    ...
    for case in base.CASES:
        scores = compute_radar_scores(summary, case)
        for method in base.METHODS:
            for (_, metric_label, _), score in zip(RADAR_METRICS, scores[method], strict=True):
                rows.append(
                    {
                        "case": case,
                        "method": method,
    ...
    with out.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=["case", "method", "metric", "score_higher_is_better"])
        writer.writeheader()
...
Figure archetype:
Four-row mixed-modality evidence wall. Version 19 keeps the accepted Version 18 panels a-h and adds one deliberately redundant radar-summary panel for integrated comparison.
...
i: summary performance profiles. Each radar converts nRMSE, RMSE, CRPS, coverage calibration, interval sharpness and seed stability to higher-is-better scores in [0, 1].

Statistics:
n=50 paired seeds per system and method. Panels e and f show seed-wise boxplots. Panels g and h show mean bars with 95% CI and overlaid seed-level dots. Panel i is a summary view, not an additional inferential test. APCE is highlighted by a red frame in statistical panels and by a framed curve in the radar panel.
"""
...
def main() -> None:
    ...
    fig = plt.figure(figsize=(11.55, 12.00))
    outer = fig.add_gridspec(
        8,
        1,
        height_ratios=[1.03, 0.13, 0.11, 0.92, 0.28, 0.92, 0.46, 1.00],
        hspace=0.22,
        left=0.050,
        right=0.994,
        top=0.955,
    ...
    qa = {
        "figure": OUTPUT_STEM,
        "layout": "version 19: preserves accepted v18 panels a-h; adds fourth-row panel i with three equal-size radar charts for Wave, Spring and Heat; radar scores are higher-is-better summaries and intentionally repeat selected information from e-h",
        "backend": "python/matplotlib",
        "font": "Arial with sans-serif fallback",
        "font_weight": "regular except panel letters",
        "formats": ["svg", "pdf", "png", "tiff"],
        "changed_from_v18": "adds panel i only; keeps no true-alpha marker, no Oracle, continuous panel labels a-i, and the d-panel right-side vertical colorbar with ticks 0 and 1 only",
        "radar_metrics": [label.replace("\n", " ") for _, label, _ in RADAR_METRICS],
    }
