#!/usr/bin/env python3
"""Cartesian PCE/APCE runner for measured acoustic source trajectories."""
...
def aggregate(output: Path, frontend: Path, segment: str) -> None:
    ...
    write_json(output / "summary.json", {
        "source_frontend": str(frontend), "segment": segment, "summaries": summaries,
    })
...
def main() -> None:
    ...
    parser.add_argument(
        "--task",
        choices=("single_source", "dual_source"),
        required=True,
    )
    parser.add_argument(
        "--protocol-config",
    ...
    protocols = json.loads(args.protocol_config.read_text(encoding="utf-8"))
    protocol = protocols[args.task]
    segment = str(protocol["segment"])
    if args.stage == "track":
        ...
        run_track(
            ...
            args.device,
            segment,
            int(protocol["ensemble_size"]),
            float(protocol["q_min_accel_mps2"]),
            float(protocol["q_max_accel_mps2"]),
            float(protocol["position_init_std_m"]),
            float(protocol["velocity_init_std_mps"]),
            float(protocol["observation_covariance_scale"]),
            float(protocol["turn_rate_radps"]),
            (
                None
                if protocol["innovation_chi2_threshold"] is None
                else float(protocol["innovation_chi2_threshold"])
            ),
        )
    else:
        aggregate(args.output / "runs", args.frontend, segment)
