...
def _load_scenario(
    ...
    *,
    ...
    if use_stored_covariance:
        ...
        # reference cases. This makes the shrinkage choice auditable and
...
def main() -> None:
    ...
    parser.add_argument("--sensor-density", type=int, default=None, help="Number of paired u/v PIV points")
    parser.add_argument("--sensor-layout", default=None, help="Prepared sensor layout id, for example 20x40")
    ...
    if args.ensemble_size is not None:
        ...
        config = dict(config)
        config["ensemble_size"] = int(args.ensemble_size)
    ...
    if args.evidence_window_frames is not None:
        ...
        config = dict(config)
        config["evidence_window_frames"] = int(args.evidence_window_frames)
    ...
    variant = args.model_variant or f"rank{int(config['rank'])}_stride1"
    ...
    ensemble_suffix = f"_ens{int(config['ensemble_size']):03d}" if args.ensemble_size is not None else ""
    ...
    run_id = f"viv_{case_id}_{args.method}_seed{args.seed:03d}" + density_suffix + layout_suffix + ensemble_suffix + covariance_suffix + shrinkage_suffix + evidence_window_suffix + ("_no_control" if args.no_control else "")
    if args.sensor_layout is not None:
        prepared = np.load(model_root / "sensor_layouts" / args.sensor_layout / f"case_{case_id}.npz", allow_pickle=False)
        sensor_points = int(prepared["sensor_observations"].shape[1] // 2)
    else:
        sensor_points = args.sensor_density or len(config["sensor_x_over_d"]) * len(config["sensor_y_over_d"])
    run_path = output_root / "runs" / f"{run_id}.json"
    trace_path = output_root / "traces" / f"{run_id}.npz"
    requested_covariance_shrinkage = float(
        config["observation_covariance_shrinkage"]
        if args.covariance_shrinkage is None else args.covariance_shrinkage
    )
    uses_stored_covariance = (
        args.observation_covariance in {"full", "tapered"}
        and args.sensor_layout is not None
        and math.isclose(
            requested_covariance_shrinkage,
            float(config["observation_covariance_shrinkage"]),
            rel_tol=0.0,
    ...
    payload: dict[str, Any] = {
        ...
        "device": str(device),
        "uses_known_cylinder_displacement_input": not args.no_control,
        "sensor_density_points": sensor_points,
        "sensor_layout": args.sensor_layout or "8x5",
        "observation_covariance": args.observation_covariance,
        "observation_covariance_shrinkage": float(
            config["observation_covariance_shrinkage"] if args.covariance_shrinkage is None else args.covariance_shrinkage
        ),
        "observation_covariance_source": "stored_archive" if uses_stored_covariance else "dynamic_training_only",
    ...
    try:
        scenario = _load_scenario(
            model_root, case_id, config, args.no_control, args.sensor_density,
            args.sensor_layout, args.observation_covariance,
        ...
        if not args.skip_full_field:
            ...
            if args.sensor_layout is not None:
                sensor = np.load(
                    model_root / "sensor_layouts" / args.sensor_layout / f"case_{case_id}.npz",
                    allow_pickle=False,
                )
                excluded = np.asarray(sensor["sensor_flat_indices"], dtype=np.int64)
            field_metrics, field_trace = full_field_metrics(
                VIVCase.open(path), PODModel.load(model_root / "pod_model.npz"),
                result.latent_estimate, device, excluded_flat_indices=excluded,
            )
            payload.update(field_metrics)
        else:
            field_trace = {"truth_energy": np.asarray([]), "predicted_energy": np.asarray([])}
        payload.update(
    ...
    except Exception as exc:  # noqa: BLE001 - persist run-level diagnostics
    ...
    payload["script_sha256"] = sha256_file(pathlib.Path(__file__).resolve())
