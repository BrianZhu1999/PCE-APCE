...
from .rom import DMDCCandidate, PODModel, evaluation_indices, fit_dmdc, model_digest, project_case, randomized_pod, sensor_flat_indices, training_mean
...
def main() -> None:
    ...
    for case in projection_cases:
        ...
        sensor_indices = sensor_flat_indices(case, config)
        mean_obs, basis_obs = pod.observation_matrix(sensor_indices)
        raw_obs = np.empty((case.time_s.size, sensor_indices.size), dtype=np.float32)
        ...
        for start, values, _valid in case.iter_physical(block=args.block):
            raw_obs[start : start + values.shape[0]] = values[:, sensor_indices]
            eval_truth[start : start + values.shape[0]] = values[:, evaluation]
        payload = {
            "coefficients": coefficients,
            "sensor_observations": raw_obs,
            "sensor_flat_indices": sensor_indices,
            "sensor_mean": mean_obs,
            "sensor_basis": basis_obs,
    ...
    manifest = {
        ...
        "pod_sha256": sha256_file(pod_path),
        ...
        "model_digest": model_digest(pod, candidates),
