...
from .io import VIVCase, list_cases, nearest_grid_indices, sha256_file, stored_memmap
...
def audit_case(path: pathlib.Path, config: dict[str, object], *, hash_file: bool) -> dict[str, object]:
    ...
    x_idx, y_idx, sensor_coords = nearest_grid_indices(
        case.x_mm,
        case.y_mm,
        list(config["sensor_x_over_d"]),
        list(config["sensor_y_over_d"]),
        float(config["cylinder_diameter_m"]),
    )
    sensor_valid = np.asarray(mask[:, y_idx[:, None], x_idx[None, :]]) > 0.5
    ...
    result: dict[str, object] = {
        ...
        "sha256": sha256_file(path) if hash_file else "not_computed",
        ...
        "sensor_x_indices": x_idx.tolist(),
        "sensor_y_indices": y_idx.tolist(),
        "sensor_coordinates_mm": sensor_coords.tolist(),
        "sensor_valid_fraction": float(sensor_valid.mean()),
    }
    return result
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Audit the public VIV-PIV NPZ collection without altering it.")
    parser.add_argument("--config", type=pathlib.Path, required=True)
    parser.add_argument("--skip-hash", action="store_true")
    args = parser.parse_args()
    config = load_config(args.config)
    data_root = pathlib.Path(config["data_root"])
    output_root = pathlib.Path(config["output_root"]) / "audit"
    ...
    records = [audit_case(paths[case_id], config, hash_file=not args.skip_hash) for case_id in sorted(paths)]
    ...
    write_json(output_root / "viv_piv_case_manifest.json", payload)
    write_csv(output_root / "viv_piv_case_manifest.csv", records)
    print(f"audited={len(records)} bytes={total_bytes} output={output_root}")
