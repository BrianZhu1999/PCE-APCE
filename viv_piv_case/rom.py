...
from .io import VIVCase, nearest_grid_indices
...
def sensor_flat_indices(case: VIVCase, config: dict[str, object], density: int | None = None) -> np.ndarray:
    x_idx, y_idx, _coords = nearest_grid_indices(
        case.x_mm,
        case.y_mm,
    ...
    spatial = [(int(iy), int(ix)) for ix in x_idx for iy in y_idx]
    if density is not None and density < len(spatial):
        selected = np.linspace(0, len(spatial) - 1, int(density), dtype=int)
        spatial = [spatial[index] for index in selected]
    width = case.x_mm.size
    flat = [(iy * width + ix) * 2 + component for iy, ix in spatial for component in (0, 1)]
...
def evaluation_indices(reference: VIVCase, config: dict[str, object], count: int) -> np.ndarray:
    ...
    sensor = set(sensor_flat_indices(reference, config).tolist())
    candidates: list[int] = []
...
def model_digest(pod: PODModel, candidates: list[DMDCCandidate]) -> str:
    digest = hashlib.sha256()
    for value in (pod.mean, pod.basis, pod.singular_values):
        digest.update(np.ascontiguousarray(value).view(np.uint8))
    for candidate in candidates:
        for value in (candidate.a, candidate.b, candidate.q_diag):
            digest.update(np.ascontiguousarray(value).view(np.uint8))
