...
LOWER_BETTER = {"nrmse", "crps", "alpha_absolute_error", "interval_width_90"}
...
def short_report(records: list[dict[str, Any]], summary: list[dict[str, Any]], paired: list[dict[str, Any]]) -> str:
    observed_points = int(records[0]["observed_points"]) if records else 64
    state_dim = int(records[0]["state_dim"]) if records else 1024
    downsample = state_dim // max(observed_points, 1)
    lines = [
        "# Figure 4 Lorenz-96-1024 scaling aggregate report",
        "",
        f"- Completed valid records: {len(records)}",
        f"- Protocol: D={state_dim}, {observed_points} uniformly spaced observed states (spatial downsampling {downsample}×), time intervals 1/2/3/4/5/6/7/8, methods Aug-EnKF/BMA/PCE/APCE.",
    ...
    for row in summary:
        lines.append(
            "- time {obs_interval}, {label}: nRMSE {nrmse_mean:.4g}, CRPS {crps_mean:.4g}, "
            "alpha MAE {alpha_absolute_error_mean:.4g}, coverage {coverage_90_mean:.4g}, width {interval_width_90_mean:.4g} "
            "(n={valid_n})".format(**row)
        )
    lines.extend(["", "## Paired focal comparisons", ""])
    for row in paired:
        lines.append(
            "- time {obs_interval}, {method_label} vs {baseline_label}: nRMSE wins {nrmse_win_count}/{paired_seed_count}, "
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Aggregate Figure 4 Lorenz-96 D=1024 scaling runs.")
    ...
    observed_points = int(records[0]["observed_points"]) if records else 64
    state_dim = int(records[0]["state_dim"]) if records else 1024
    run_csv = source / "lorenz96_1024_run_source_data.csv"
    summary_csv = source / "lorenz96_1024_method_summary.csv"
    paired_csv = source / "lorenz96_1024_paired_gains.csv"
    failure_csv = source / "lorenz96_1024_failure_records.csv"
    report_path = source / "L96_1024_SCALING_REPORT.md"
