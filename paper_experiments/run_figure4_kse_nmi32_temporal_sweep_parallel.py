...
DEFAULT_OUTPUT = ROOT / "results" / "figure4_kse_nmi_official_sparse32_t2to8_5seeds_20260814_core4"
DEFAULT_METHODS = ("pce", "apce", "aug_enkf", "bma_static")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Parallel KSE 32x spatial + temporal sparsity sweep.")
    ...
    intervals = [int(x) for x in args.temporal_intervals.split(",") if x.strip()]
    methods = [x.strip() for x in args.methods.split(",") if x.strip()]
    devices = [x.strip() for x in args.devices.split(",") if x.strip()]
    tasks: list[dict[str, Any]] = []
    for temporal in intervals:
        for seed_index in range(int(args.seed_count)):
            sample_index = int(seed_index)
            for method in methods:
                run_id = f"kse_nmi{args.downsampling_factor}x_t{temporal}_{method}_seed{seed_index:02d}_sample{sample_index:02d}"
                tasks.append(
                    {
                        "run_id": run_id,
