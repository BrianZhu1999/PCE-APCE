...
core = importlib.import_module("paper_experiments.run_figure4_kolmogorov64_velocityobs")
...
def aggregate(output: Path) -> None:
    ...
    failure_csv = source / "kolmogorov64_blackout_failure_records.csv"
    write_csv(
        failure_csv,
        [row for row in scalar_rows if row.get("status") != "completed" or not row.get("valid")],
    )
    manifest = {
        "protocol": "kolmogorov64_velocityobs_re1500_k2_s16_t4_blackout40_v1",
...
def run_one(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if args.record_trace:
            ...
            payload["trace_sha256"] = core.file_sha256(trace_path)
    ...
    payload["runner_sha256"] = core.file_sha256(Path(__file__).resolve())
    write_json(run_path, payload)
    print(json.dumps(clean_json(payload), ensure_ascii=False))


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="KOL-64 blackout forecast worker")
