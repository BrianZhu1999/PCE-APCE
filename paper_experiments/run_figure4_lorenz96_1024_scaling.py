...
TUNING_PROFILE_OVERRIDES: dict[str, dict[str, Any]] = {
    "baseline": {},
    "pce_temp_020": {"pce_temperature": 0.20},
    "pce_temp_026": {"pce_temperature": 0.26},
    "pce_temp_030": {"pce_temperature": 0.30},
    "apce_floor_045": {"apce_entropy_floor": 0.45},
    "apce_floor_055": {"apce_entropy_floor": 0.55},
    "apce_floor_062": {"apce_entropy_floor": 0.62},
    "alpha_conservative": {
    ...
    "apce_score_separation": {"apce_recycle_entropy_projected_scores": False},
...
def dynamic_grid_coordinates(
    ...
    """Use the current branch alpha means as regridding coordinates when requested.

    The default implementation keeps static grid labels for exact backward
    compatibility. The dynamic option is a separately audited tuning variant:
    it only replaces stale branch labels with their own current posterior means,
    never with the hidden alpha truth.
    """
...
def apply_tuning_profile(config: L96ScalingConfig, profile_text: str) -> L96ScalingConfig:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Independent Figure 4 Lorenz-96 D=1024 worker.")
