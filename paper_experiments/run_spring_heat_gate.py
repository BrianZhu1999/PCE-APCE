...
def gate_decisions(summary: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for case in ("spring", "heat"):
        ...
        for method in ("pce", "apce"):
            ...
            oracle_gap = case_rows[method]["nrmse"] - case_rows["oracle_alpha"]["nrmse"]
            decisions.append(
                {
                    "case": case,
                    "method": method,
                    ...
                    "nrmse_excess_over_oracle": float(oracle_gap),
                }
            )
    return decisions
...
def write_report(output: Path, summary: list[dict[str, Any]], decisions: list[dict[str, Any]]) -> None:
    lines = [
        "# Spring/Heat APCE-PCE 快速闸门报告",
        "",
        "## 结论口径",
        "",
        "本报告只用于低成本快速筛选，不替代 20 配对种子的正式确认。主判据是 APCE/PCE 是否在同一真值、观测、初始 ensemble 和 forecast noise 下同时降低 nRMSE 与 CRPS。",
        "",
        "## 均值指标表",
        "",
    ...
    for row in summary:
        lines.append(
            "| {case} | {label} | {nrmse:.4%} | {rmse:.6f} | {crps:.6f} | {coverage_90:.2%} | {interval_width_90:.6f} | {alpha_absolute_error:.5f} | {runtime_seconds:.3f} |".format(**row)
        )
    lines.extend(["", "## 快速闸门判定", ""])
    for decision in decisions:
        status = "通过" if decision["quick_gate_pass"] else "失败"
        lines.append(
            f"- {decision['case']} / {decision['method'].upper()}：{status}；"
            f"nRMSE 赢全部 fixed baseline = {decision['wins_all_fixed_baselines_on_nrmse']}，"
            f"CRPS 赢全部 fixed baseline = {decision['wins_all_fixed_baselines_on_crps']}，"
            f"相对 Oracle-alpha 的 nRMSE 额外误差 = {decision['nrmse_excess_over_oracle']:.6f}。"
        )
    lines.append("")
    (output / "SPRING_HEAT_APCE_PCE_GATE_REPORT.md").write_text("\n".join(lines), encoding="utf-8")
...
def run_suite(n_seeds: int, base_seed: int, output: Path, device: torch.device) -> dict[str, Any]:
    ...
    payload = {
        ...
        "cases": ["spring", "heat"],
        "case_configs": {
            case: asdict(config_for_case(case, base_seed)) for case in ("spring", "heat")
        },
        "summary": summary,
        "decisions": decisions,
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="5-seed Spring/Heat APCE-PCE quick gate.")
    parser.add_argument("--n-seeds", type=int, default=5)
    parser.add_argument("--base-seed", type=int, default=2026080600)
    parser.add_argument("--output", type=Path, default=Path("results_spring_heat_gate_5seeds"))
    parser.add_argument("--device", default="cpu")
    args = parser.parse_args()
    device = torch.device(args.device)
    result = run_suite(args.n_seeds, args.base_seed, args.output, device)
